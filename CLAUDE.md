...
- **Memory** (`nanobot/agent/memory.py`): Session history persistence with Dream two-phase memory consolidation. In multi-user mode, private sessions scope both Dream writes and prompt loading to user-specific `USER_<user_id>.md` / `USER_PRIVATE_<user_id>.md` files. Uses atomic writes with fsync for durability.
